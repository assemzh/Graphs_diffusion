...
def get_heat_matrix(
        ...
        if kernel == 'heat':
            ...
            D_tilde = np.diag(1/np.sqrt(A_tilde.sum(axis=1)))
            H = D_tilde @ A_tilde @ D_tilde
            return expm(-t * (np.eye(num_nodes) - H)) #heat
        elif kernel == 'SCCT_H':
            I = np.eye(num_nodes)
            d = np.sum(adj_matrix, axis=0).reshape((-1, 1))
            D05 = np.diag(np.power(d, -0.5)[:, 0])
            H2 = np.eye(num_nodes) - np.ones((num_nodes, num_nodes)) / num_nodes
            volG = np.sum(adj_matrix)
            M = D05.dot(adj_matrix - d.dot(d.transpose()) / volG).dot(D05)
            K_CCT = H2.dot(D05).dot(M).dot(np.linalg.pinv(I - M)).dot(M).dot(D05).dot(H2)
            sigma2 = K_CCT.std()
            Kds2 = K_CCT / sigma2
            return 1. / (1. + np.exp(-0.05 * Kds2)) # SCCT_H
        elif kernel == 'CCT_H':
            I = np.eye(num_nodes)
            d = np.sum(adj_matrix, axis=0).reshape((-1, 1))
            D05 = np.diag(np.power(d, -0.5)[:, 0])
...
def get_Katz_matrix(
    ...
    """
    num_nodes = adj_matrix.shape[0]
    return np.linalg.pinv(np.eye(num_nodes) - t * adj_matrix)
